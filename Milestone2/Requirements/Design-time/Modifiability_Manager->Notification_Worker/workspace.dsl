    ...
    model {
        ...
        healthExa = softwareSystem "Exam Handling System (EXA)" "Manages exam terms, registrations, rooms, results and statistics." {
            ...
            notificationWorker = container "Notification Worker" "Asynchronously builds and sends e-mail notifications and logs delivery." "Background worker" {
                ...
                notificationDispatcher   = component "Notification Dispatcher" "Routes notifications to delivery channels and persists delivery status."
        ...
        registrationNotifier    -> notificationDispatcher "Submits registration notifications to"   "In-process call"
        roomReservationNotifier -> notificationDispatcher "Submits reservation notifications to"    "In-process call"
        resultNotifier          -> notificationDispatcher "Submits result notifications to"         "In-process call"

        notificationDispatcher  -> emailSender      "Uses for e-mails via"                          "In-process call"
        notificationDispatcher  -> notificationLog  "Writes notification log entry via"             "In-process call / JDBC"
