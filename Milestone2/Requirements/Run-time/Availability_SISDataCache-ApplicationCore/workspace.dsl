workspace "Exam Handling System" "C4 model for EXA project" {
    ...
    model {
        ...
        healthExa = softwareSystem "Exam Handling System (EXA)" "Manages exam terms, registrations, rooms, results and statistics." {
            ...
            // ----- Container: SIS Sync Worker -----
            sisSyncWorker = container "SIS Sync Worker" "Periodically fetches SIS master-data needed by EXA and refreshes the cache snapshots." "Background worker" {
                snapshotScheduler = component "Snapshot Scheduler" "Triggers periodic refresh of SIS master-data snapshots."
                sisClient         = component "SIS Client" "Fetches required SIS master-data (courses, enrolment/credit status, requirements)."
                cacheWriter       = component "Cache Writer" "Writes snapshots to the SIS Data Cache and stores the snapshot timestamp."
            }
            ...
            // ----- Container: SIS Data Cache -----
            sisDataCache = container "SIS Data Cache" "Stores periodic snapshots of the subset of SIS master data needed by EXA (max age 24h) to allow fallback when SIS is unavailable." "Redis cache" {
                tags "Database"
                snapshotStore = component "Snapshot Store" "Key/value snapshot storage with a snapshot timestamp."
            }
        }
        ...
        appCore -> sisDataCache "Reads cached SIS master-data snapshots when SIS is unavailable (fallback)" "Redis/TCP"
        sisSyncWorker -> sis "Fetches SIS master-data snapshots" "REST/HTTPS"
        sisSyncWorker -> sisDataCache "Stores refreshed snapshots and snapshot timestamp" "Redis/TCP"
        ...
        deploymentEnvironment "Dev/Test" {
            deploymentNode "Developer Laptop" "Local development or test environment" "Docker / local runtime" {
                ...
                containerInstance sisSyncWorker
                deploymentNode "Local DB" "Test database" "PostgreSQL" {
                    containerInstance exaDb
                }
                deploymentNode "Local Cache" "Local SIS snapshot cache" "Redis" {
                    containerInstance sisDataCache
                }
            }
        ...
        deploymentEnvironment "Production" {
            ...
            deploymentNode "Kubernetes Cluster" "Production app cluster" "Kubernetes" {
                ...
                deploymentNode "Sync Pod" "Pod hosting the SIS sync worker (periodic snapshot refresh)." "Kubernetes Pod" {
                    containerInstance sisSyncWorker
                }
            }
            deploymentNode "Cache Server" "Managed SIS snapshot cache" "Redis" {
                containerInstance sisDataCache
            }
