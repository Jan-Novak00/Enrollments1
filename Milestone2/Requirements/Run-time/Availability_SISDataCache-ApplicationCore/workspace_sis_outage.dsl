workspace "Exam Handling System" "C4 model for EXA project" {
    ...
    model {
        ...
        healthExa = softwareSystem "Exam Handling System (EXA)" "Manages exam terms, registrations, rooms, results and statistics." {
            ...
            sisCache = container "SIS Data Cache" "Caches selected SIS master data snapshots (course requirements, enrolment status) so EXA can operate during SIS outages." "Key-value store (Redis)" {
                tags "Cache"
            }

            // ----- Container: SIS Sync Worker -----
            sisSyncWorker = container "SIS Sync Worker" "Periodically refreshes SIS master data snapshots into the SIS Data Cache." "Background worker" {
                tags "Worker"
            }
        ...
        appCore -> sisCache "Reads cached SIS snapshots (fallback during SIS outages)" "Redis"
        sisSyncWorker -> sis "Fetches master data snapshots" "REST/DB"
        sisSyncWorker -> sisCache "Updates cached snapshots" "Redis"
        ...
        deploymentEnvironment "Production" {
            ...
            deploymentNode "Kubernetes Cluster" "Production app cluster" "Kubernetes" {
                ...
                deploymentNode "SIS Sync Pod" "Pod hosting the SIS sync worker." "Kubernetes Pod" {
                    containerInstance sisSyncWorker
                }
            }
            deploymentNode "Cache Cluster" "Managed in-memory cache for SIS snapshots" "Redis" {
                containerInstance sisCache
            }
