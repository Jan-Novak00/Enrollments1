    ...
    model {
        ...
        deploymentEnvironment "Production" {
            deploymentNode "Kubernetes Cluster" "Production app cluster" "Kubernetes" {
                ...
                deploymentNode "Student Registration Load Balancer" "Load balances student exam registration requests for Application Core"  {
                    registrationLoadBalancer = infrastructureNode "Load balancer" "" "" "performance"
                }

                deploymentNode "App Pod" "Pod hosting the EXA application core." "Kubernetes Pod" "" 5 {
            ...
            deploymentNode "Registration Queue Manager" "" "Apache Kafka" {
                registrationRequestsQueue = infrastructureNode "Registration Requests Queue" "A queue of the different requests for exam registration" "" "performance"
                registrationResultsQueue = infrastructureNode "Registration Results Queue" "A queue of the results for the different exam registration requests"  "" "performance"   
            }
            ...
            webAppInstance -> registrationLoadBalancer "Sends requests for application use cases"
            registrationLoadBalancer -> appCoreInstance "Sends student exam registration requests to"

            appCoreInstance -> registrationRequestsQueue "Sends registration request requests"
            appCoreInstance -> registrationResultsQueue "Picks registration results"
            exaDbInstance -> registrationRequestsQueue "Picks registration request for processing"
            exaDbInstance -> registrationResultsQueue "Sends registration request results"
        }
    }
